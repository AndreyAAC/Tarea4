...
public class InventoryController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var summaries = inventories.Select(inv => new SummaryViewModel
            {
                Id = inv.InventoryId,
                Name = $"Inventory {inv.InventoryId}",
                Value = $"{inv.UnitsInStock} units • {inv.UnitPrice}",
                Count = 1
            }).ToList();
        ...
        catch (Exception ex)
        {
            ...
            return View(new InventoryPageViewModel { Inventories = [], Summaries = [] });
        }
    }
