...
public class ComponentController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var response = await _restProvider.GetAsync(endpoint, (string?)null);

            var components = JsonProvider.DeserializeSimple<List<ComponentViewModel>>(response)
                             ?? new List<ComponentViewModel>();

            var summaries = components.Select(c => new SummaryViewModel
            {
                Id = c.Id,                        
                Name = c.Name,                    
                Value = c.Content,                
            ...
            var viewModel = new ComponentPageViewModel
            {
        ...
        catch (Exception ex)
        {
            ...
            return View(new ComponentPageViewModel
            {
                Components = [],
                Summaries = []
            });
