...
public class SupplierController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var summaries = suppliers.Select(s => new SummaryViewModel
            {
                Id = s.SupplierId,
                Name = s.SupplierName ?? "(no name)",
                Value = s.Phone ?? s.City ?? "",
                Count = 1
            }).ToList();
        ...
        catch (Exception ex)
        {
            ...
            return View(new SupplierPageViewModel { Suppliers = [], Summaries = [] });
        }
    }
