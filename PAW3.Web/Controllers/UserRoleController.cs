...
public class UserRoleController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var summaries = userRoles.Select(ur => new SummaryViewModel
            {
                Id = ur.Id,
                Name = $"User {ur.UserId}",
                Value = $"Role {ur.RoldId}",
                Count = 1
            }).ToList();

            var vm = new UserRolePageViewModel
            {
                UserRoles = userRoles,
                Summaries = summaries
            };
        ...
        catch (Exception ex)
        {
            ...
            return View(new UserRolePageViewModel { UserRoles = [], Summaries = [] });
        }
    }
    ...
    public async Task<IActionResult> Edit(int id, UserRoleViewModel userRole)
    {
        try
        {
            if (id != (int?)userRole.Id) return NotFound();
